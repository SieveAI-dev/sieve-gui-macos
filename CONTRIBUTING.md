...
A mock daemon harness is not wired up yet; integration testing connects to a real `sieve` daemon over `~/.sieve/ipc.sock`. With the daemon absent, the GUI enters the `disconnected` state. See [`docs/guides/development.md`](docs/guides/development.md) for the harness plan.
...
If your change interacts with any of the above, link the relevant ADR/SPEC in your PR description and explain how the constraint is preserved.
