...
> Sieve GUI 是 [Sieve daemon](https://github.com/SieveAI-dev/sieve) 的 native macOS 守门人壳。daemon 做检测，GUI 只做交互。
...
- **平台**：macOS 版本 / 芯片架构（Intel / Apple Silicon）/ 安装方式（DMG / 源码构建）
- **漏洞类型**（任一）：
...
> 如涉及**当前正在被利用 + 用户资产损失风险**，请在标题加 `[URGENT]`，将优先响应。
...
- **daemon 检测漏报 / 误报**：见 [sieve daemon SECURITY.md](https://github.com/SieveAI-dev/sieve/blob/main/SECURITY.md)
...
GUI 与 daemon 共享相同的可验证供应链承诺：

- 所有 release 二进制 **Apple Developer ID 签名 + Notarization**
...
- [Sieve daemon SECURITY.md](https://github.com/SieveAI-dev/sieve/blob/main/SECURITY.md) — daemon 侧安全策略
