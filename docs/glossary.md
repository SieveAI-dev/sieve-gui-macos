...
> 不在此列的上游术语，请去 daemon 仓库的 PRD v2.0 §2 查（见 [`external/upstream-references.md`](external/upstream-references.md)）。
...
### AutoRedact
处置类型之一：daemon 检测到出站匹配后，**自动改写**请求 body（如把 API key 替换为 `[REDACTED]`），不弹窗、不阻断。GUI 收 `event_notify` 后只显示 Toast。
...
### critical_lock
PRD v2.0 引入的规则保护机制：被 lock 的规则**禁止**`allow_remember`、禁止用户改 `timeout_seconds` 和 `default_on_timeout`。GUI 侧的执行职责见 [`SPEC-002`](specs/SPEC-002-hips-popup-window.md)。
...
### evidence_meta
audit.db 中的字段：命中证据的元数据（`{"len": 412, "prefix_hash": "...", ...}`）。**不含原始字节**——这与 PRD §11.2 不存原文的承诺一致。
...
### LAContext
`LocalAuthentication.framework` 的认证上下文。Sieve GUI 用它做 Touch ID 解锁敏感字段（[ADR-008](design/adr/ADR-008-touchid-unlock-session.md)）。
...
### protocol_version
IPC 协议版本号。当前 `v1`。GUI 不识别版本号 → 进入 disconnected。
