...
> 版本：v1.0
> 适用：daemon 与 GUI 对齐 SPEC-005 IPC 协议 v2 的版本
...
**代码位置：**
- daemon 侧握手构造：[`crates/sieve-ipc/src/socket_server.rs`](../../../sieve/crates/sieve-ipc/src/socket_server.rs)（`// SPEC-005 §3：连接建立后第一条出站消息` 注释处）
- daemon 7 字段参数结构：[`crates/sieve-ipc/src/socket_server.rs`](../../../sieve/crates/sieve-ipc/src/socket_server.rs)（`HelloParams` 组装，`protocol_version: "v2"`）
- GUI 侧握手解析：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`handleHello` 方法）
- GUI 侧支持版本集合：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`supportedProtocolVersions: Set<String> = ["v2"]`）
- GUI 侧握手完成通知 delegate：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`ipcDidHandshake`）
- GUI 侧模型定义（7 字段）：[`Sources/Models/HelloParams.swift`](../../Sources/Models/HelloParams.swift)
...
**代码位置：**
- daemon 侧心跳间隔常量：[`crates/sieve-ipc/src/socket_server.rs`](../../../sieve/crates/sieve-ipc/src/socket_server.rs)（`HEARTBEAT_INTERVAL_SECS: u64 = 25`）
- daemon 侧心跳发送逻辑：[`crates/sieve-ipc/src/socket_server.rs`](../../../sieve/crates/sieve-ipc/src/socket_server.rs)（`heartbeat_interval.tick()` 分支）
- GUI 侧超时检测：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`checkHeartbeat`，超时阈值 30s）
- GUI 侧心跳不记录到 IPC Monitor（过滤噪音）：[`Sources/Services/IPC/IPCRouter.swift`](../../Sources/Services/IPC/IPCRouter.swift)（`if method != "sieve.heartbeat" { ... }`）
...
- 若 IPC Monitor 里出现大量 heartbeat 记录：`IPCRouter.swift` 过滤逻辑失效
...
**代码位置：**
- GUI 侧版本检查：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`guard IPCClient.supportedProtocolVersions.contains(hello.protocolVersion) else`）
- GUI 侧进入 terminal 状态：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`state = .versionMismatch(received:)`，`shouldReconnect = false`）
- GUI 侧 inflight 全部失败：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`inflight.failAll(error: .versionMismatch)`）
- GUI 侧状态枚举：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`case versionMismatch(received: String)`）
...
**失败时：**
- 若 GUI 仍在重连：`IPCClient.swift`（`scheduleRetry` 里的 versionMismatch guard）失效
...
**代码位置：**
- HIPS 弹窗管理器入口（`enqueueRequest`）：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)
- 弹窗呈现逻辑（`present`）：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)
- 弹窗视图主体：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`body`）
- Header（标题 + SeverityChip + 方向 + 倒计时）：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)
- SeverityChip 组件：[`Sources/UI/Components/SeverityChip.swift`](../../Sources/UI/Components/SeverityChip.swift)
- IPC 路由到 HIPS：[`Sources/Services/IPC/IPCRouter.swift`](../../Sources/Services/IPC/IPCRouter.swift)（`handleDaemonRequest` → `sieve.request_decision`）
...
- 弹窗不居中：`HipsPanelManager.swift`（`centerOnActiveScreen`）
...
**代码位置：**
- 多 issue 渲染入口：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`if request.merged { ForEach(request.issues) { issue in IssueCardView(...) } }`）
- `IssueCardView` 组件：[`Sources/Features/HIPS/DetailCardView.swift`](../../Sources/Features/HIPS/DetailCardView.swift)
- merged critical issue → 隐藏"全部允许"按钮：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`shouldHideAllowAll`）
...
**代码位置：**
- deny 时记录时间：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)（`denyTracker.recordDeny(ruleId:)`）
- 判断是否互换：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)（`denyTracker.shouldSwapLayout(ruleId:)`）
- 互换时间窗口常量：[`Sources/Models/HipsDenyTracker.swift`](../../Sources/Models/HipsDenyTracker.swift)（`swapWindowSeconds: TimeInterval = 5`）
- 互换布局渲染：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`if swappedLayout { ... }`）
...
**代码位置：**
- 红阶段判断：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`phaseRequiresCmdClick`，`phase == .red`）
- phase 计算：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`currentPhase`，`r > 0.2 → orange`，否则 `red`）
- tryAllow 里 ⌘ 检查：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`if !flags.contains(.command) { return }`）
- 400ms 防误触 swallow：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)（`isClickSwallowed`，400ms 内返回 true）
- 允许按钮 label 变化：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`allowLabel`，红阶段显示"按住 ⌘ 点击允许"）
...
需要 daemon 发送 `allow_remember: false` 的 `request_decision`。目前 Critical 规则默认 `allow_remember: false`（产品红线）。
...
**代码位置：**
- footer 渲染层红线：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`if request.allowRemember { Toggle(...) } else { Image(systemName: "lock.fill") }`）
- 编码层强制（决策时）：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)（`let safeRemember = req.allowRemember ? remember : false`）
- `DecisionResponse.resultJSON` 编码层最终强制：[`Sources/Models/DecisionResponse.swift`](../../Sources/Models/DecisionResponse.swift)（`"remember": allowRemember ? remember : false`）
...
**失败时：**
- 若 checkbox 灰显可见：`HipsPopupView.swift` 的 if 分支错误
- 若锁图标不出现：同上，else 分支
...
**代码位置：**
- 主按钮锁判定逻辑：[`Sources/Models/Recommendation.swift`](../../Sources/Models/Recommendation.swift)（`mainActionLocksToDeny`：`guard let rec else { return true }`，`rec.confidence != .high`）
- 弹窗使用此判定：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`var mainActionLocked: Bool { Recommendation.mainActionLocksToDeny(request.recommendation) }`）
- 锁时按钮布局：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`if mainActionLocked || phaseRequiresCmdClick` 分支，"允许"降级为 `.bordered`）
...
- 规则定义：[`crates/sieve-rules/rules/inbound.toml`](../../../sieve/crates/sieve-rules/rules/inbound.toml)（pattern 匹配 `eth_signTypedData_v4` 等）
...
**代码位置：**
- 按钮渲染（DEBUG only）：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`.overlay(alignment: .topTrailing) { if request.rawJSON != nil { Button(...) } }`）
- 复制 + 5s 清空逻辑：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`copyRawJSON`，`Task.sleep(5s)` 后 `NSPasteboard.clearContents`）
- 确认 alert：[`Sources/Features/HIPS/HipsPopupView.swift`](../../Sources/Features/HIPS/HipsPopupView.swift)（`.alert("原始 JSON 含敏感字段", isPresented: $showCopyJSONAlert)`）
...
**代码位置：**
- reduce-motion 设置：[`Sources/Features/Settings/GeneralSettingsView.swift`](../../Sources/Features/Settings/GeneralSettingsView.swift)（`Picker("减少动效", selection: $appState.settings.reduceMotionOverride)`）
...
**代码位置：**
- 重连后关闭所有 stale 弹窗：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)（`closeAllActiveDialogs`）
- IPCRouter 触发关闭：[`Sources/Services/IPC/IPCRouter.swift`](../../Sources/Services/IPC/IPCRouter.swift)（`ipcDidDiscardInflightOnReconnect` → `hipsManager?.closeAllActiveDialogs()`）
- disconnectedCache 字段（冗余防丢）：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)
...
**代码位置：**
- 前置校验：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)（`guard req.merged || req.context != nil else { handleRenderFailure(...) }`）
- `handleRenderFailure` 实现：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)
...
**代码位置：**
- Custom 模式规则表格：[`Sources/Features/Settings/DetectionPresetView.swift`](../../Sources/Features/Settings/DetectionPresetView.swift)（`customRuleTable`）
...
**代码位置：**
- GraylistSheetView：[`Sources/Features/Settings/PrivacyDataSettingsView.swift`](../../Sources/Features/Settings/PrivacyDataSettingsView.swift)
- `sieve.list_graylist` 调用：[`Sources/Features/Settings/PrivacyDataSettingsView.swift`](../../Sources/Features/Settings/PrivacyDataSettingsView.swift)
- `sieve.remove_graylist` 调用：[`Sources/Features/Settings/PrivacyDataSettingsView.swift`](../../Sources/Features/Settings/PrivacyDataSettingsView.swift)
...
**代码位置：**
- 图标渲染（bind appState.daemonStatus）：[`Sources/Features/MenuBar/MenuBarController.swift`](../../Sources/Features/MenuBar/MenuBarController.swift)（`bindAppState`）
...
**代码位置：**
- 角标更新（bind holdRemainingSeconds）：[`Sources/Features/MenuBar/MenuBarController.swift`](../../Sources/Features/MenuBar/MenuBarController.swift)（`updateHoldBadge`，`button.title = " " + "\(secs)s"`）
...
**代码位置：**
- warning 角标更新：[`Sources/Features/MenuBar/MenuBarController.swift`](../../Sources/Features/MenuBar/MenuBarController.swift)（`updateWarningBadge`，`count > 99 ? "99+" : "\(count)"`）
...
**代码位置：**
- 暂停请求（强制 bounded 1~30）：[`Sources/Features/MenuBar/MenuBarController.swift`](../../Sources/Features/MenuBar/MenuBarController.swift)（`requestPause`，`let bounded = max(1, min(30, minutes))`）
- 乐观更新：[`Sources/Features/MenuBar/MenuBarController.swift`](../../Sources/Features/MenuBar/MenuBarController.swift)
- 恢复：[`Sources/Features/MenuBar/MenuBarController.swift`](../../Sources/Features/MenuBar/MenuBarController.swift)（`requestResume`，`minutes: 0`）
...
**代码位置：**
- 确认逻辑：[`Sources/Features/MenuBar/MenuBarController.swift`](../../Sources/Features/MenuBar/MenuBarController.swift)（`confirmQuit`，NSAlert + `NSApp.terminate`）
...
**代码位置：**
- disconnected 专用视图：[`Sources/Features/MenuBar/QuickMenuView.swift`](../../Sources/Features/MenuBar/QuickMenuView.swift)（`if case .disconnected(let reason) = appState.daemonStatus { disconnectedSection(reason: reason) }`）
...
**代码位置：**[`Sources/Features/Settings/GeneralSettingsView.swift`](../../Sources/Features/Settings/GeneralSettingsView.swift)
...
**代码位置：**[`Sources/Features/Settings/DetectionPresetView.swift`](../../Sources/Features/Settings/DetectionPresetView.swift)
...
**代码位置：**[`Sources/Features/Settings/PrivacyDataSettingsView.swift`](../../Sources/Features/Settings/PrivacyDataSettingsView.swift)
...
**代码位置：**[`Sources/Features/Settings/DaemonSettingsView.swift`](../../Sources/Features/Settings/DaemonSettingsView.swift)
...
**代码位置：**[`Sources/Features/Settings/UpdatesSettingsView.swift`](../../Sources/Features/Settings/UpdatesSettingsView.swift)
...
**代码位置：**[`Sources/Features/Settings/UpdatesSettingsView.swift`](../../Sources/Features/Settings/UpdatesSettingsView.swift)（`AboutSettingsView`）
...
**代码位置：**
- HistoryWindowView：[`Sources/Features/History/HistoryWindowView.swift`](../../Sources/Features/History/HistoryWindowView.swift)
- InspectorPanelView：[`Sources/Features/History/InspectorPanelView.swift`](../../Sources/Features/History/InspectorPanelView.swift)
...
**代码位置：**
- 200ms debounce：[`Sources/Features/History/HistoryWindowViewModel.swift`](../../Sources/Features/History/HistoryWindowViewModel.swift)（`$keywordInput.debounce(for: .milliseconds(200)...)`）
...
**代码位置：**
- 分页：[`Sources/Features/History/HistoryWindowViewModel.swift`](../../Sources/Features/History/HistoryWindowViewModel.swift)（`loadMore`，offset 分页）
- 增量推送（file watcher）：[`Sources/Features/History/HistoryWindowViewModel.swift`](../../Sources/Features/History/HistoryWindowViewModel.swift)（`reader.startWatching { appendIncremental() }`）
...
**代码位置：**
- TouchIDService 5min 会话：[`Sources/Services/TouchID/TouchIDService.swift`](../../Sources/Services/TouchID/TouchIDService.swift)（`appState.setUnlockSession(UnlockSession())`）
- 锁屏清除：[`Sources/Services/TouchID/TouchIDService.swift`](../../Sources/Services/TouchID/TouchIDService.swift)（`observeScreenLock`）
- Inspector 解锁 UI：[`Sources/Features/History/InspectorPanelView.swift`](../../Sources/Features/History/InspectorPanelView.swift)
...
**代码位置：**
- 导出按钮：[`Sources/Features/History/HistoryWindowView.swift`](../../Sources/Features/History/HistoryWindowView.swift)（`exportButton`）
- 进度条：[`Sources/Features/History/HistoryWindowView.swift`](../../Sources/Features/History/HistoryWindowView.swift)（`exportProgressBar`）
...
- 确认 dialog 强制脱敏提示：[`Sources/Features/History/HistoryWindowView.swift`](../../Sources/Features/History/HistoryWindowView.swift)（Text: "历史记录将强制脱敏，不含 evidence 原文"）
...
**代码位置：**
- Inspector 重放按钮：[`Sources/Features/History/InspectorPanelView.swift`](../../Sources/Features/History/InspectorPanelView.swift)（`WindowManager.shared.replayInDebug(prompt:)`）
- DebugReplayStore：[`Sources/App/DebugReplayStore.swift`](../../Sources/App/DebugReplayStore.swift)
- RuleEvaluationTab 监听 replayStore：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)（`applyPrefilledIfNeeded`）
...
**代码位置：**
- LiveEventsTab：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)
- grep 200ms debounce：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)（`DispatchQueue.main.asyncAfter(deadline: .now() + 0.2, execute: work)`）
- 暂停按钮：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)（`buffer.paused.toggle()`，ring buffer 继续记录）
...
**代码位置：**
- RuleEvaluationTab：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)
- 64KB 限制（UI + IPC）：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)（`payload.utf8.count > 65536` 时按钮 disabled 且文字变红）
- evaluate IPC 调用：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)（`sieve.evaluate`）
...
**代码位置：**
- IPCMonitorTab：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)
- params 不展示（硬红线）：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)（`Text("params: 不展示")`）
- 详情面板 params 红线说明：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)（`Text("params 不展示（SPEC-005 红线）")`）
...
**代码位置：**
- SystemStatusTab：[`Sources/Features/Debug/DebugWindowView.swift`](../../Sources/Features/Debug/DebugWindowView.swift)（大约）
...
**代码位置：**
- OnboardingView：[`Sources/Features/Onboarding/OnboardingView.swift`](../../Sources/Features/Onboarding/OnboardingView.swift)
- 6 步标题：[`Sources/Features/Onboarding/OnboardingView.swift`](../../Sources/Features/Onboarding/OnboardingView.swift)（`["欢迎", "环境检查", "通知权限", "开机启动", "Preset 选择", "完成"]`）
...
**代码位置：**
- 合并逻辑：[`Sources/Features/Toast/ToastController.swift`](../../Sources/Features/Toast/ToastController.swift)（`if let existingIdx = stack.firstIndex(where: { $0.kind == params.kind && $0.ruleId == params.ruleId && Date().timeIntervalSince($0.firstSeenAt) < 5 })`）
...
**代码位置：**
- 上限判断：[`Sources/Features/Toast/ToastController.swift`](../../Sources/Features/Toast/ToastController.swift)（`if stack.count >= 3 { return }`）
...
**代码位置：**
- 时长使用：[`Sources/Features/Toast/ToastController.swift`](../../Sources/Features/Toast/ToastController.swift)（`let duration = TimeInterval(appState.settings.toastDurationSeconds)`）
...
**代码位置：**
- `handleTap` 回调：[`Sources/Features/Toast/ToastController.swift`](../../Sources/Features/Toast/ToastController.swift)（`ToastView(entry: entry, onTap: { handleTap(entry) }, ...)`）
...
**代码位置：**
- 重连 Toast：[`Sources/Features/Toast/ToastController.swift`](../../Sources/Features/Toast/ToastController.swift)（`presentReconnect`）
...
- IPCRouter 三路判定：[`Sources/Services/IPC/IPCRouter.swift`](../../Sources/Services/IPC/IPCRouter.swift)（`if let kind = self.appStateAdapter?.checkAndUpdateDaemonBootId(params.daemonBootId) { self.toastController?.presentReconnect(kind) }`）
...
**代码位置：**
- inflight 清空（不重发）：[`Sources/Services/IPC/IPCClient.swift`](../../Sources/Services/IPC/IPCClient.swift)（`inflight.clearAndDiscard()`）
...
**代码位置：**
- 关闭 stale 弹窗入口：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)（`closeAllActiveDialogs`）
...
**代码位置：**
- 调用：[`Sources/Features/Settings/DetectionPresetView.swift`](../../Sources/Features/Settings/DetectionPresetView.swift)（`if !rulesUnavailable && liveRules.isEmpty { Task { await refreshRules() } }`）
- 错误降级：[`Sources/Features/Settings/DetectionPresetView.swift`](../../Sources/Features/Settings/DetectionPresetView.swift)（`rulesUnavailable: Bool`，`rulesError: String?`）
...
**代码位置：**
- 调用：[`Sources/Features/Settings/PrivacyDataSettingsView.swift`](../../Sources/Features/Settings/PrivacyDataSettingsView.swift)（`sieve.purge_history`）
...
- 触发需要 daemon 的 `address_guard`（近似地址检测），需真实 EVM 地址对比
- 测试方式：在 inbound 响应中包含两个仅有细微差异的近似以太坊地址
...
**代码位置：**
- 弹窗关闭时清空 rawJSON：[`Sources/Features/HIPS/HipsPanelManager.swift`](../../Sources/Features/HIPS/HipsPanelManager.swift)（`activeRequest?.clearRawJSON()`）
...
- Inspector 中使用：[`Sources/Features/History/InspectorPanelView.swift`](../../Sources/Features/History/InspectorPanelView.swift)
...
**代码位置：**
- second-pass 逻辑：[`crates/sieve-cli/src/engine_adapter.rs`](../../../sieve/crates/sieve-cli/src/engine_adapter.rs)
- checksum 验证：[`crates/sieve-cli/src/engine_adapter.rs`](../../../sieve/crates/sieve-cli/src/engine_adapter.rs)（`if bip39::verify_checksum(&window, wl) { ... 定级 Critical ... }`）
...
**错误码源码位置：**
- daemon 侧：[`crates/sieve-ipc/src/error.rs`](../../../sieve/crates/sieve-ipc/src/error.rs)（`pub mod rpc_codes`）
- GUI 侧：[`Sources/Models/DecisionResponse.swift`](../../Sources/Models/DecisionResponse.swift)（`DecisionError` enum，-32100~-32102）
