...
> Status: Stable
> Owner: SieveAI
> 上游依赖：[上游 SPEC-003 sieve-setup-tool](../external/upstream-references.md#spec-003sieve-setup-tool) · [上游 sieve-setup-tool 决策](../external/upstream-references.md#sieve-setup-tool)
...
**多 agent 接入**：`sieve setup` 支持配置多个 AI agent（`--agent claude|openclaw|hermes|codex` 可重复，或 `--all-detected` 自动检测已安装的 agent）。上方检查项展示的是 Claude Code（`ANTHROPIC_BASE_URL` + PreToolUse hook）路径；其他 agent 经各自的 base_url 配置注入与 hook 接入，doctor 会针对每个已配置的 agent 给出对应检查行。
...
| 指标 | 约束 |
|------|------|
| 首次安装完成 | 5 分钟内完成全流程（含 sieve setup 耗时）|
| 权限申请 | 必须走系统标准 API，不允许 osascript 绕开 |
| 登录项 | `SMAppService`（macOS 13+），不用老 API |
