...
- HIPS 弹窗（见 SPEC-002）
- 菜单栏图标状态（见 SPEC-001）
- Toast 内容的原始数据解析（数据由 `sieve.notify_status_bar` 携带）
...
### 场景 A：单次 AutoRedact
```
daemon 发 notify_status_bar{kind:"outbound_redacted", rule_id:"OUT-01", summary:"Anthropic API key"}
...
### 场景 B：5 秒内多次命中（合并）
```
5s 内 3 次 notify_status_bar（OUT-01 × 2 + OUT-09 × 1）
...
| 事件类型 | 图标 | 背景色 |
|---------|-----|------|
| `kind="outbound_redacted"` | 盾牌 | 橙色警告底 |
...
**合并规则**：5 秒内同类型（同 `kind`，如 `outbound_redacted`）的 event → 合并到已有 Toast，更新计数 + 副文。不同 `kind` 分开显示。
...
Toast 触发由 `sieve.notify_status_bar` 驱动，详见：
- [ipc-protocol §3.3](../api/ipc-protocol.md#33-sievenotify_status_bar通知)
...
| 指标 | 约束 |
|------|------|
| Toast 渲染延迟 | < 100ms（NSPanel 复用）|
| Toast 内容 | 不展示原始命中内容 |
| 堆叠上限 | ≥3 个时改用角标，避免刷屏 |
| 系统通知内容 | 仅 rule_id 等元信息，不含证据细节 |
| ToastController | 单例（`NSPanel` 复用）|

---

## 8. 测试要求

- 单次 `notify_status_bar{kind:"outbound_redacted"}` → Toast 出现，5s 后消失
